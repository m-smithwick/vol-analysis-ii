...
def calculate_forward_returns(df: pd.DataFrame, periods: List[int] = [1, 5, 10, 20]) -> pd.DataFrame:
    """
...
def generate_backtest_report(df: pd.DataFrame, ticker: str, period: str) -> str:
    ...
    report_lines.append(f"📊 BACKTEST ANALYSIS FOR {ticker.upper()} ({period})")
    report_lines.append("=" * 70)
    report_lines.append("")
    ...
    report_lines.append("🚪 EXIT SIGNAL PERFORMANCE:")
    report_lines.append("")
    ...
    if exit_results:
        for signal_col, metrics in exit_results.items():
            ...
            if metrics['accuracy'] >= 70:
                report_lines.append(f"    ✅ Highly reliable exit indicator")
            elif metrics['accuracy'] >= 50:
                report_lines.append(f"    ✓ Moderately reliable exit indicator")
            else:
                report_lines.append(f"    ⚠️  Less reliable - use with caution")
    ...
    if best_signal:
        report_lines.append(f"  ✅ Most Reliable Signal: {best_signal} ({best_win_rate:.1f}% win rate)")
    
    if system_metrics and system_metrics['win_rate'] >= 60:
        report_lines.append(f"  ✅ System shows positive edge (>{system_metrics['win_rate']:.0f}% win rate)")
    elif system_metrics and system_metrics['win_rate'] >= 50:
        report_lines.append(f"  ⚠️  System is marginally profitable ({system_metrics['win_rate']:.0f}% win rate)")
    elif system_metrics:
        report_lines.append(f"  ❌ System underperforming ({system_metrics['win_rate']:.0f}% win rate)")
    
    if system_metrics and system_metrics['sharpe_ratio'] >= 1.0:
        report_lines.append(f"  ✅ Good risk-adjusted returns (Sharpe: {system_metrics['sharpe_ratio']:.2f})")
    ...
    if total_exit_signals > 0:
        ...
        if avg_exit_accuracy >= 60:
            report_lines.append(f"  ✅ Exit signals are reliable ({avg_exit_accuracy:.0f}% accuracy)")
    
    # Statistical significance warning
    if system_metrics and system_metrics['valid_signals'] < 20:
        report_lines.append("")
        report_lines.append(f"  ⚠️  WARNING: Limited sample size ({system_metrics['valid_signals']} signals)")
    ...
    report_lines.append("  • Win Rate >60% = Strong signal")
    report_lines.append("  • Expectancy >1.5% = Profitable signal")
    report_lines.append("  • Sharpe Ratio >1.0 = Good risk-adjusted returns")
    report_lines.append("  • Exit Accuracy >60% = Reliable exit timing")
    report_lines.append("")
...
def generate_strategy_comparison_report(paired_trades: List[Dict], 
    ...
    report_lines.append("=" * 70)
    report_lines.append("🎯 ENTRY-TO-EXIT STRATEGY ANALYSIS")
    report_lines.append("=" * 70)
    report_lines.append("")
    ...
    report_lines.append("📊 OVERALL TRADING STATISTICS:")
    ...
    if closed_trades == 0:
        report_lines.append("⚠️  No closed trades to analyze.")
        report_lines.append("   Generate more signals or use a longer time period.")
        return "\n".join(report_lines)
    
    # Entry strategy comparison
    report_lines.append("🚀 ENTRY STRATEGY COMPARISON:")
    report_lines.append("")
    ...
    if entry_comparison:
        ...
        for signal_col, metrics in sorted_entries:
            ...
            if metrics['win_rate'] >= 70 and metrics['expectancy'] > 2.0:
                report_lines.append(f"    ✅ EXCELLENT - Highly profitable strategy")
            elif metrics['win_rate'] >= 60 and metrics['expectancy'] > 1.0:
                report_lines.append(f"    ✅ GOOD - Strong positive edge")
            elif metrics['win_rate'] >= 50 and metrics['expectancy'] > 0:
                report_lines.append(f"    ✓ FAIR - Marginally profitable")
            else:
                report_lines.append(f"    ❌ POOR - Needs improvement")
            
            report_lines.append("")
    
    # Exit strategy comparison
    report_lines.append("🚪 EXIT STRATEGY COMPARISON:")
    report_lines.append("")
    ...
    if exit_comparison:
        ...
        for signal_col, metrics in sorted_exits:
            ...
            if metrics['win_rate'] >= 70:
                report_lines.append(f"    ✅ EXCELLENT exit timing")
            elif metrics['win_rate'] >= 55:
                report_lines.append(f"    ✓ GOOD exit timing")
            elif metrics['win_rate'] >= 45:
                report_lines.append(f"    ⚠️  FAIR - Consider alternatives")
            else:
                report_lines.append(f"    ❌ POOR - Avoid this exit")
            
            report_lines.append("")
    
    # Best combination analysis
    report_lines.append("⭐ OPTIMAL STRATEGY COMBINATIONS:")
    report_lines.append("")
    ...
    # Statistical significance warning
    if closed_trades < 10:
        report_lines.append("⚠️  WARNING: Limited sample size!")
...
def run_backtest(df: pd.DataFrame, ticker: str, period: str, 
    ...
    """
    Run complete backtest analysis on signal DataFrame.
    
    Generates two reports:
    1. Traditional forward returns analysis (fixed time periods)
    2. Entry-to-exit paired analysis (actual trading performance)
    ...
    Returns:
        str: Combined backtest report text
    """
    import os
    
    # Calculate forward returns
    df_with_returns = calculate_forward_returns(df, periods=[1, 5, 10, 20])
    
    # Generate traditional forward returns report
    forward_report = generate_backtest_report(df_with_returns, ticker, period)
    ...
    combined_report = forward_report + "\n\n" + strategy_report
    ...
    if save_to_file:
        ...
        print(f"\n💾 Backtest report saved: {filepath}")
        print(f"   📊 Forward Returns Analysis")
        print(f"   🎯 Entry-to-Exit Strategy Analysis")
