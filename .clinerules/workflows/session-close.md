...
1. **Read** `project_state.md` and `SESSION_IMPROVEMENTS_SUMMARY.md`.
...
3. **Clear** `project_state.md` so only the skeleton headers remain (Context, Architectural Impact, Plan, Janitor Queue).
...
5. **Review** the "Janitor Queue" from the old `project_state.md`.
   - If items are critical/blocking: Add them to the NEW (blank) `project_state.md` as the first tasks.
